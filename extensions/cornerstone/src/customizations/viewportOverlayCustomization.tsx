...
/**
 * Per-viewport view-label detector.
 *
 * Returns a short clinical view label for the overlay (e.g. "RCC", "LMLO",
 * "Axial T1", "Sagittal Post", "MIP", "Coronal", "R LONG"). Detection is
 * tolerant to missing tags and dummy SeriesDescription values — it tries
 * multiple DICOM sources in priority order and degrades to `null` if it
 * cannot decide. A `null` return makes the overlay row disappear entirely
 * (via the customization `condition` callback), so a detection miss can
 * never produce a misleading or broken label.
 *
 * Source priority mirrors the hanging-protocol display-set selectors in
 * `extensions/default/src/hangingprotocols/utils/{mammo,mr}DisplaySetSelector.ts`,
 * keeping the labeling consistent with how the protocols choose viewports.
 */
function getViewLabel(instance: any): string | null {
  if (!instance) return null;
  try {
    const modality = String(instance.Modality || '').toUpperCase();
    if (modality === 'MG') return getMammoViewLabel(instance);
    if (modality === 'MR' || modality === 'MRI') return getMRViewLabel(instance);
    if (modality === 'US') return getUSViewLabel(instance);
...
function getMammoViewLabel(instance: any): string | null {
  ...
  if (!view && instance.ViewCodeSequence) {
    ...
    if (code === '399162004' || code === 'R-10242') view = 'CC';
    else if (code === '399368009' || code === 'R-102D2') view = 'MLO';
  }
  ...
  if (!view) {
    const tokens = ['XCCL', 'XCCM', 'LMLO', 'RMLO', 'LCC', 'RCC', 'MLO', 'CC', 'ML', 'LM', 'FB', 'AT'];
    for (const t of tokens) {
      if (desc.includes(t)) {
        view = t.length === 4 ? t.slice(1) : t; // 'LMLO' → 'MLO', 'RCC' → 'CC'
        if (!lat && t.length === 4) lat = t[0]; // recover laterality from prefix
        if (!lat && (t === 'LCC' || t === 'RCC')) lat = t[0];
        break;
      }
    }
  }
  ...
  if (!lat) {
    if (/\b(L|LT|LEFT)\b/.test(desc) || /^L[\s_-]/.test(desc)) lat = 'L';
    else if (/\b(R|RT|RIGHT)\b/.test(desc) || /^R[\s_-]/.test(desc)) lat = 'R';
  }
  ...
  let label = '';
  if (lat && view) label = `${lat}${view}`;
  else if (view) label = view;
  else if (lat) label = lat;
...
function getMRViewLabel(instance: any): string | null {
  ...
  if (plane && sequence) return `${plane} ${sequence}`;
...
function getMRSequence(instance: any): string | null {
  ...
  if (matchesImageType(instance.ImageType, ['MIP']) || /\bMIP\b/.test(desc)) return 'MIP';
  if (matchesImageType(instance.ImageType, ['SUBTRACTION']) || /\bSUB\b/.test(desc)) return 'SUB';
  if (/\bSTIR\b/.test(desc)) return 'STIR';
  if (/\bFLAIR\b/.test(desc)) return 'FLAIR';
  if (/\b(DWI|DIFFUSION)\b/.test(desc)) return 'DWI';
  if (/\bADC\b/.test(desc)) return 'ADC';
  if (/\b(POST|DCE|GD|DELAYED)\b/.test(desc)) return 'T1 Post';
  if (/\bT2\b|T2W/.test(desc)) return 'T2';
  if (/\bT1\b|T1W/.test(desc)) {
    if (/\b(PRE|BASELINE)\b/.test(desc)) return 'T1 Pre';
    return 'T1';
  }
...
function getUSViewLabel(instance: any): string | null {
  ...
  if (/\b(TRANS|TRV|TRANSVERSE|AXIAL|AX)\b/.test(desc)) plane = 'TRANS';
  else if (/\b(LONG|LONGITUDINAL|SAG|SAGITTAL)\b/.test(desc)) plane = 'LONG';

  if (lat && plane) return `${lat} ${plane}`;
  if (plane) return plane;
  if (lat) return `${lat} US`;
  return null;
}
...
function getPlaneFromIOP(iop: any): string | null {
  ...
  if (arr.length < 6 || arr.some(isNaN)) return null;
